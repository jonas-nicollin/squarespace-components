...
(function () {
  ...
  function buildMobilePanel(appendSecondary, tabPool, i18n, blockKey) {
    ...
    function open() {
      ...
      if (sourceBlock) {
        ...
        // Propager les classes sqb-- et sqb-tab-- du bloc sur le panneau
        // → permet le ciblage CSS depuis le panneau (ex: .sqb-tab--expositions .sqb-filter-group)
        var classesToPropagate = Array.from(sourceBlock.classList).filter(function(c) {
          return c.indexOf('sqb--') === 0 || c.indexOf('sqb-tab--') === 0;
        });
        // Retirer les anciennes classes propagées
        Array.from(panel.classList).forEach(function(c) {
          if (c.indexOf('sqb--') === 0 || c.indexOf('sqb-tab--') === 0) panel.classList.remove(c);
        });
        classesToPropagate.forEach(function(c) { panel.classList.add(c); });
      }
  ...
  async function runConfig(cfg) {
    ...
    function onTabChange(tab) {
      ...
      // Mettre à jour les classes tab sur le panneau mobile si ouvert
      if (mobileObj && mobileObj.panel) {
        var p = mobileObj.panel;
        Array.from(p.classList).forEach(function(c) {
          if (c.indexOf('sqb-tab--') === 0) p.classList.remove(c);
        });
        var tabLabel = tab.label || '';
        if (tabLabel) p.classList.add('sqb-tab--' + tabLabel.toLowerCase()
          .normalize('NFD').replace(/[\u0300-\u036f]/g, '')
